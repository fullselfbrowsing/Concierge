...
const REQUIRED_CLOSURE_TASKS = Object.freeze([
  ...
  Object.freeze({
    ...
    threat: "T-06-G05",
    secureBehaviorTokens: Object.freeze([
      `${EXPECTED_M06_IDS.length}-row register`,
      "range self-tests",
      "ledger self-tests",
    ]),
  ...
  Object.freeze({
    ...
    threat: "T-06-G05",
    secureBehaviorTokens: Object.freeze([
      `${EXPECTED_M06_IDS.length}/${EXPECTED_M06_IDS.length}`,
      "verify all",
    ]),
    testType: "mutation",
...
const MUTANTS = Object.freeze([
  ...
  runtimeMutant({
    id: "M-06-S37",
    group: "single",
    name: "aliased argument graphs receive a shared fallback key",
    target: "packages/concierge/src/dispatch.ts",
    literalPattern: lines(
      "  if (seen.has(value)) {",
  ...
  runtimeMutant({
    ...
    replacement: "  for (const call of snapshotToolCalls(batch)) {",
    intendedCaseIds: ["Q02"],
  }),
  runtimeMutant({
    id: "M-06-B02",
    ...
    target: "packages/concierge/src/dispatch.ts",
    literalPattern: "    return left.originalIndex - right.originalIndex;",
    replacement: "    return right.originalIndex - left.originalIndex;",
    intendedCaseIds: ["Q02"],
  }),
  runtimeMutant({
    id: "M-06-B03",
    ...
    replacement: lines(
      "  const rawCalls = callsRead.value as Array<{ readonly outputIndex?: number }>;",
      "  rawCalls.sort(",
      "    (left, right): number =>",
  ...
  runtimeMutant({
    ...
    replacement: lines(
      ...
      "      const argumentsMalformed: boolean = false;",
    ),
    intendedCaseIds: ["Q04"],
  }),
  runtimeMutant({
    ...
    target: "packages/concierge/src/dispatch.ts",
    literalPattern: "        argumentsMalformed = true;",
    replacement: "        argumentsMalformed = false;",
    intendedCaseIds: ["Q04"],
  }),
  runtimeMutant({
    id: "M-06-B07",
  ...
  runtimeMutant({
    ...
    target: "packages/concierge/src/dispatch.ts",
    literalPattern: "    rows.push(batchRow(call.callId, result));",
  ...
  runtimeMutant({
    ...
    target: "packages/concierge/src/dispatch.ts",
    literalPattern: "    rows.push(batchRow(call.callId, result));",
    replacement: "    rows.push(batchRow(undefined, result));",
    intendedCaseIds: ["Q07"],
  }),
  runtimeMutant({
    id: "M-06-B17",
    ...
    replacement: "  return { callId, result };",
    intendedCaseIds: ["Q08"],
  }),
  runtimeMutant({
    id: "M-06-B18",
  ...
  runtimeMutant({
    ...
    replacement: lines(
      ...
      "        call.name,",
      "        args,",
      "        { ...meta, callId: `${meta.callId}:${meta.outputIndex}` },",
      "        argumentsMalformed,",
      "      );",
    ),
    intendedCaseIds: ["Q13"],
...
function selfTestLedgerSnapshot() {
  ...
  const closureRows = REQUIRED_CLOSURE_TASKS.map((task) => {
    const evidence = {
      ...
      "06-08-T1": `${EXPECTED_M06_IDS.length}-row register with range self-tests and ledger self-tests`,
      "06-08-T2": `${EXPECTED_M06_IDS.length}/${EXPECTED_M06_IDS.length} plus verify all`,
...
function selfTest() {
  ...
  const ledgerCounterexamples = [
    ...
    ["stale detector count", (value) => {
      value.validationText = value.validationText.replace(
        `${EXPECTED_SINGLE_IDS.length}/${EXPECTED_SINGLE_IDS.length} single`,
        `${EXPECTED_SINGLE_IDS.length - 1}/${EXPECTED_SINGLE_IDS.length} single`,
      );
      return value;
    }],
