...
- [ ] 04-03-PLAN.md — Wave 2: **THE HINGE** — `src/concierge.ts` (`createConcierge`, one flat `buildCatalog`, the **index-keyed** instance-local memo, the shallow projection freeze over shared deep-frozen elements, one guarded `runMatch`, one-pass `explain`, the `reason`-omitting `dispatch` stub), the barrel, and the export surface moved 59/49/10 → 62/51/11 with both its pins (STG-01/02/03/04, SEC-03, DX-01, CAT-03)
...
- [ ] 04-08-PLAN.md — Wave 5: phase gate — all seven gate scripts, the `dist/index.d.ts` prose audit, a byte-identical `pnpm-lock.yaml`, and the validation sign-off
