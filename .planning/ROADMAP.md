...
- **(a) `ActionResult.reason` is still open `string`.** Resolved as a pure closed union — `reason?: ReasonCode | undefined`, thirteen codes, with `AbandonReason` reused as a named subset rather than deleted. The explicit `| undefined` is required by this repo's `exactOptionalPropertyTypes`. `message` stays unbranded but gains a declared length bound and a documented never-a-consent-artifact policy.
