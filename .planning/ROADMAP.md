...
**Success Criteria** (what must be TRUE):
  1. A transport can report that a readback was cut off partway — the delivery hook carries a completion outcome, so partial delivery is representable instead of indistinguishable from completion. Applies to any interruption: a dismissed surface, a navigation, a disconnect, a spoken interruption.
...
**Notes**: Scope is the verified remainder, not all sixteen defects SUMMARY listed. Ten are already fixed in the committed `types.ts` — the `ToolBatch` envelope, the regraded `ConsentGrade`, `respond(ActionResult)`, ordered stage array, `SnapshotNormalizer`, `Session`, the deleted `registerHandler`, the `jsonSchema?` escape hatch, the object-rooted `JsonSchemaObject`, and the memoized-`catalogFor` contract. Two more were closed after this roadmap was drafted: `deferUntilDelivered` now passes a `DeliveryReport` with an explicit `outcome`, making partial delivery representable, and `ConsentPolicy` no longer threads the action's own `Name` (it was inferring as a union of the action and its `requires` target and corrupting the name-union derivation) — it takes `string` and CAT-03 checks it at build time, where the catalog actually exists. The remainder: (a) `ActionResult.reason` is still open `string`; (b) no readback sink type, so `readbackHash` has no producer; (c) no server consent-token shape; (d) no composition or taint metadata (`readsUntrusted`, `maxPerTurn`, `conflictsWith`, `impact`). Cheap additions belonging here: a `Snapshot` type parameter on `ActionDefinition` (without it `snapshotEquality` degrades to `unknown` exactly where correctness matters), a `scheduler?` seam, and `stage`/`onStageChange` on `Session`. `CONTRACT_VERSION` is deliberately *not* here — it is purely additive and lands in Phase 2 where the test that needs it lives.
