...
| **Measured final runtime** | 16 runtime files / 331 passed / 331 total / 0 pending / 0 todo (`pnpm test`, exit 0) |
...
|-----|--------|------------------|-----------------------|--------|
| T-07-01 | Reentrant transition resumes stale catalog/context authority, mistakes confirmed authority for transport reality, or admits work before reconciliation | Serialized context/status transition drain, exact first-failure preservation with queued-control continuation, exact failed-request rollback to confirmed-replay authority, latest-generation checkpoints after every resolver/capability property read and invocation, queued and active requested-generation admission binding, separate publishing/published/confirmed catalog state, identity-based epoch promotion/abort, publication-gated pump, fixed-catalog fail-close, and abandoned-attempt cleanup after hostile accessor reentry | C01-C22 plus connected-replay getter regressions; M-07-C01..C16 and M-07-R03..R04 all compiled, ran named detectors, and were killed | ✅ mitigated |
| T-07-02 | Concurrent/retried routing duplicates work or responses | Session-wide FIFO, one `dispatchBatch` per accepted occurrence, one non-retried response attempt per row, including progress after an accessor-abandoned, current-exception, failed-request replay, or resolver/capability-superseded publication | C11-C22, J01-J06, J15-J18; M-07-C05/C06/C09/C10/C11/C12/C13/C14/C15/C16 and M-07-R01/R06/R07/R08/R09 | ✅ mitigated |
...
| T-07-04 | Transition, stop, or subscriber reentrancy leaves live state | Transition queue/generation guard; exact queued, active, and post-failure effective authority ownership; guarded outside resolver/capability boundaries; mark stopped and invalidate drain before outside calls; tokenized listeners; independent cleanup; no post-stop output | C10-C22, L01-L13; M-07-C05..C16 and M-07-L01..L08 | ✅ mitigated |
...
| 07-07-01 | 07 | 6 | SES-01, SES-02 | T-07-01, T-07-02 | Hostile setTools and resolver/capability boundaries that queue C abandon B completely, retain queued and active C authority after queue shift, preserve exact current throws while draining connected replay, reconcile an exact failed request to coherent confirmed-replay authority, and bind batches to exact arrival generation/context across return/throw, distinct/same-catalog, supersession, repeated-generation, connected/disconnected replay, failure chains, no-context, and stop variants | integration + concurrency + security | `pnpm --filter @fullselfbrowsing/concierge build && pnpm exec vitest run packages/concierge/test/session-catalog.test.ts --testNamePattern="^\\[C(17|18|19|20|21|22)\\]"` | ✅ `packages/concierge/src/session.ts`, `packages/concierge/test/session-catalog.test.ts`; C17-C22 | ✅ green |
| 07-07-02 | 07 | 6 | SES-01, SES-02 | T-07-01, T-07-02 | Independent compiled C17 abort/clear, C18 stale-boundary, C19 exceptional-progress, C20 queued-request authority, C21 active-request authority, and C22 failed-request reconciliation mutants plus complete regenerated revision-bound mutation evidence | mutation + concurrency | `node scripts/phase-07-mutation-battery.mjs verify all && node scripts/phase-07-mutation-battery.mjs verify inputs` | ✅ register/evidence; M-07-C10..M-07-C16 and 37/37 green | ✅ green |
...
| Reconnect | first connected; reconnect after another status; repeated identical-status control suppressed by the stub; replay getter queues C before returning or throwing, with zero stale invocation/fatal error and final C authority; failed C queues one or multiple confirmed-A replays across connected/disconnected mixtures without resurrecting C |
| Transition reentrancy | nested context C from old-epoch abort; nested C from successful setTools(B); nested C from the B setTools accessor where C reuses already-published A; nested C from catalogFor/stageFor property capture or invocation and from capabilities/dynamicCatalog reads immediately before return or throw; active C work emitted from those same six boundaries after C is shifted from the queue; exact failed C rollback before confirmed-A replay with nested same/distinct D, same-object later generations, chained D failure/replay/E, and direct/signal stop; C→D supersession, repeated C generation, connected replay/status reentry, and direct/signal-accessor stop; B skipped before publication vs successfully published but not confirmed; distinct C, C catalog identical to published B, and C catalog identical to pre-transition confirmed A; histories A→C, A→B→C, A→B, and A→B→A; final newest context/catalog/stage plus exact accessor-time C dispatch/response authority |
| Publication admission | batch emitted before a setTools getter queues C, another emitted after C is queued, batches emitted from active C after its queue shift, and batches emitted inside successful setTools with and without nested C; zero stale A fallback for queued/active C work; exact provisional and requested-generation cancellation response under C; zero premature handler/output; distinct/return-to-A C aborts B, same-published-B C promotes B epoch; exact held/later dispatch, handler, response, arrival-context, generation, epoch-state, and finalizer counts |
...
- [x] `packages/concierge/test/session-catalog.test.ts` is present and C01-C22 are named, discovered, and green.
...
- [x] `scripts/phase-07-mutation-battery.mjs` plus immutable register/evidence artifacts are present; the register digest is `58e8e7d6f15a61156d4f9cc8acad2a86af7840b860f4d2107c6fda261bbd004f` and all 37 rows are green.
...
The mutation harness must contain exactly 37 ordered mutants (16 catalog, 9 routing, 8 lifecycle, 2 diagnostic, 2 package/guard) and prove each mutant built and its named detector executed; a build failure alone is not behavioral proof.

Execution uses exactly eleven contiguous same-group shards of at most four rows: C01-C04, C05-C08, C09-C12, C13-C16, R01-R04, R05-R08, R09-R09, L01-L04, L05-L08, D01-D02, and P01-P02.

Final gap-closure mapping, with a 37-row total and 16/9/8/2/2 distribution:
...
- M-07-C13 adds only an exceptional-drain break after the first preserved boundary failure. It compiles, runs exactly C19, and is killed only by `[RED:C19:current-exception-drain-progress]`, whose six built-artifact boundary variants require exact thrown identity plus queued replay and later work progress before stop.
- M-07-C14 disables only the pre-publication requested-transition admission branch. It compiles, runs exactly C20, and is killed only by `[RED:C20:post-request-admission-authority]`, whose 24 boundary variants plus supersession/stop cases require immutable C generation/context ownership, cancellation, FIFO responses, and exact finalization.
- M-07-C15 disables only the queue-empty active requested-generation authority branch. It compiles, runs exactly C21, and is killed only by `[RED:C21:active-request-generation-authority]`, whose 20 reachable resolver/capability variants plus direct/reentrant, supersession, repeated-generation, connected-replay, and direct/signal-stop cases require exact C context identity, cancellation, FIFO responses, and finalization without A fallback.
- M-07-C16 disables only the exact context-and-generation failure-reconciliation branch. It compiles, runs exactly C22, and is killed only by `[RED:C22:failed-request-authority-reconciliation]`, whose built-artifact matrix requires confirmed-A authority before a nested D request, exact D authority afterward, no failed-C resurrection, no newer-generation overwrite, exact first thrown identity, connected/disconnected replay progress, and true-arrival stop detachment for direct, signal-accessor, same-object, chained-failure, no-work, and no-context cases.
- M-07-C08 retains its unchanged latest-generation mutation and required C10/C11/C16 coverage; its fourth detector is now C20 rather than C15 because exact requested-generation binding makes C15 invariant under that mutant while C20 directly detects the removed guard.
...
- Stop the outer transition drain after preserving a current boundary failure, leaving queued connected replay blocked.
- Bind boundary-time post-request work to confirmed authority instead of exact requested generation/context.
- Bind work emitted from an active requested transition after its queue shift to confirmed authority instead of the exact active generation/context.
- Leave an exact failed requested context as effective replay authority instead of reconciling it to the confirmed context while preserving the monotonic generation.
...
|----------|-----------------|
| Immutable register | Digest `58e8e7d6f15a61156d4f9cc8acad2a86af7840b860f4d2107c6fda261bbd004f` |
| Distribution | 16 catalog / 9 routing / 8 lifecycle / 2 diagnostics / 2 package-guard (`16/9/8/2/2`) |
| Outcome | 37/37 green; zero pending, zero escaped, zero failed |
...
| Bounded execution | Exactly eleven contiguous shards: C01-C04, C05-C08, C09-C12, C13-C16, R01-R04, R05-R08, R09-R09, L01-L04, L05-L08, D01-D02, P01-P02 |
...
Immutable snapshot release revision: digest `b6dd1789125cc1f5b1a5cfdd3f22ac4f057decadeccb8fa7d817ef70c681a1cb`, generated 2026-08-10 UTC; all seven commands ran against the same read-only snapshot with stable pre/around/post digests.

| Gate | Measured result |
|------|-----------------|
| `pnpm build` | Exit 0 |
| `pnpm typecheck` | Exit 0 |
| `pnpm test` | Exit 0; 16 runtime files, 331 passed, 331 total, 0 pending, 0 todo |
...
- [x] Approval records the actual UTC date, matching register digest, 37/37 outcome, and green release gate.

**Approval:** approved 2026-08-10 — register 58e8e7d6f15a61156d4f9cc8acad2a86af7840b860f4d2107c6fda261bbd004f; 37/37 green; release gate green
