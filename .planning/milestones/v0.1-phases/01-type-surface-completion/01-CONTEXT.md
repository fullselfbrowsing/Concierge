...
event name in core, plus **TRN-05** — turn-identity provenance on `TransportCapabilities`, added to
this phase on 2026-07-27 (see D-10; it is the one item here that is unambiguously breaking to defer).
...
| `FailureReason` (machine-caused) | `invalid_args`, `invalid_result`, `unknown_action`, `no_bridge`, `handler_error`, `aborted`, `consent_required`, `consent_stale`, `grade_unavailable` |

**`invalid_result` was added 2026-07-27** after reading a second prior implementation (see
`<canonical_refs>` → Prior implementations). Its `CONTROL_FAILURE_MESSAGES` map has six categories;
five map onto the codes above, and the sixth — *the handler returned something that is not a valid
result* — had no equivalent here. That system carries a whole `normalizeControlResult(value: unknown)`
boundary because handlers return junk in practice. Our `ActionHandler` return type enforces the
shape at compile time, but the dispatcher receives whatever actually comes back at runtime: a
JavaScript consumer, a handler that falls off the end returning `undefined`, a promise that resolves
to a string. DSP-09 is the Phase 6 runtime half.

> ⚠️ **Count corrected 2026-07-28 by the plan checker.** This paragraph previously ended "Thirteen
> codes, not twelve." **The correct total is TWELVE** — 3 `AbandonReason` + 9 `FailureReason`, exactly
> as the table above lists. Two compounding errors produced the wrong word: the original table header
> said "twelve" over a list of *eleven*, and adding `invalid_result` was then described as taking it
> to thirteen. Membership was always right; only the count word was wrong. It propagated to
> ROADMAP.md, 01-RESEARCH.md, 01-VALIDATION.md, and eight places in 01-02-PLAN.md — where it was a
> hard acceptance criterion, so an executor would have had to either invent a thirteenth code
> (corrupting a union this phase declares final) or violate a stated criterion. All corrected.
> **The number is twelve. Do not "restore" thirteen.**
...
`message` is spoken or rendered, and handlers return unbounded, control-character-laden strings. So:

- **Phase 1** exports `MESSAGE_MAX_CHARS` as a constant alongside `USER_CANCELLED` / `USER_DECLINED`,
  and states the bound in the `ActionResult.message` doc comment. A constant, not a branded type —
  the branding argument above still holds.
- **Phase 6** implements the sanitizer at the dispatcher boundary. That is SEC-06.

This does not weaken D-02's core claim. `message` stays plain `string` and stays a
never-a-consent-artifact; sanitizing it makes it *safe to render*, not *trustworthy as consent*.
...
**The sink uses the generic-function form** `<P>(rb: Readback<P>) => Promise<ReadbackReceipt>`,
following `SnapshotNormalizer`'s shape, rather than a defaulted generic alias
`ReadbackSink<Payload = unknown>`.

> ⚠️ **Corrected 2026-07-28 by Phase 1 research — the original justification here was wrong.**
> This decision previously claimed a typed app sink `(rb: Readback<Booking>) => …` "fails to assign
> to the defaulted alias (TS2322)." A prototype disproved it: **neither form accepts a
> payload-specific app sink** — that was never the discriminator. The real, testable difference is
> that a defaulted alias accepts a type argument and a generic function does not
> (`ReadbackSink<Booking>` → TS2315). The conclusion survives; the reasoning behind it did not, and
> the type test must assert TS2315 rather than the assignability failure originally described.

**`DigestLike` must use method syntax, not function-property syntax.** Also from research:
function-property syntax fails against real Node `webcrypto.subtle`, whose `BufferSource` differs
from the DOM lib's. This makes `DigestLike` the deliberate **opposite** of `snapshotEquality`, which
must stay function-property — under method syntax bivariance silently un-breaks the very defect the
snapshot test exists to catch. Two adjacent seams, two opposite syntaxes, both load-bearing. The
plan must not "harmonize" them.
...
Use **`tsc --noEmit` over `*.test-d.ts` files**. Zero new dependencies, and it keeps the two phases
genuinely disjoint. Vitest's `expectTypeOf` would create a real Phase 1 → Phase 2 dependency that
the roadmap says does not exist.

> ⚠️ **Corrected 2026-07-28 by Phase 1 research — `@ts-expect-error` is the wrong default.**
> This decision originally specified `@ts-expect-error` as the assertion mechanism. Research
> demonstrated it suppresses *any* error on the following line: a directive written to prove a bad
> `reason` is rejected was satisfied by two unrelated typos and passed green. TypeScript has no
> error-code scoping — `// @ts-expect-error TS9999` still suppresses a TS2322.
>
> **Use predicate assertions instead** — `Expect<Not<Assignable<…>>>`. They model
> `exactOptionalPropertyTypes` correctly and name the invariant in the echoed source line, so a
> failure says *which* guarantee broke. Reserve `@ts-expect-error` for object-literal freshness,
> which predicates cannot model.
>
> **Mutation testing is mandatory, not optional.** Research reintroduced all ten corrected defects
> against a working prototype. Seven were caught by the obvious assertions; **three escaped** — the
> `ReadbackSink` form, the `ToolBatch` delivery hook, and `ActionDefinition.handler` dropping its
> type arguments. Each needed a purpose-built assertion. A plan that says "write type tests for the
> success criteria" without naming those three ships a green suite over a broken contract.
>
> **Compiler-version independence is confirmed.** Every experiment was re-run under TypeScript 7.0.2
> alongside the installed 5.9.3 with byte-identical diagnostics, so Phase 2's compiler bump cannot
> invalidate the suite. Full typecheck is ~0.2s (5.9.3) / ~0.08s (7.0.2) — there is no case for
> sampling less than everything on every commit.
...
The suite must assert at minimum the three regressions Success Criterion **7** names (this decision
originally said "Criterion 5" — criteria 5 and 6 were inserted during the 2026-07-27 correction pass
and the type-test criterion moved to 7; content unchanged, numbering moved):
...
> ⚠️ **Added 2026-07-28 by Phase 1 research — this edit does not compile on its own.**
> Threading `Snapshot` breaks `types.ts`: `ActionDefinition<…, Booking, …>` is not assignable to
> `ActionDefinition<…, unknown, unknown>` (TS2375, verified), so **`StageDefinition.actions` and
> `ConciergeConfig.crossStage` both fail to compile**. An erased collection view — an
> `AnyActionDefinition` — must land in the *same* edit, not a follow-up. Both `any`-erasure and
> `never`-erasure were verified to work.
>
> **Decision: use `any`-erasure**, with a doc comment explaining why the `any` is deliberate and
> load-bearing rather than laziness. The tradeoff surfaces in Phases 4, 6, and 8 (where the catalog
> and dispatcher consume the erased view) and should be revisited in Phase 8 against a real kernel.
...
**Added 2026-07-27, after this discussion had otherwise concluded.** This is the only finding in the
set that is a genuine post-publish breaking change, and it is a security hole rather than a
tidiness issue.

`TransportCapabilities.userTurnIdentity: boolean` (`types.ts:443`) records *whether* a transport can
derive turn identity. It cannot record *where that identity comes from* — and the difference is
load-bearing.

**The failure.** On a voice transport, `userTurnId` is derived from speech-recognizer turn
boundaries. The microphone picks up the assistant's own text-to-speech output, and the recognizer
transcribes it as user speech. **The agent's own output therefore mints a new user turn** — which is
precisely what `bindTo: "userTurn"` accepts as proof that a human acted. The gate the entire design
rests on is satisfied by the agent talking to itself, with no human involved at any point.

**Why PITFALLS P2 does not cover this.** P2 is a *human* barging in: they truncate the readback and
mint a genuinely new user turn in one gesture. Its prescribed mitigation is turn classification — "a
turn whose transcript is `stop` / `wait` / `no` must not confirm." That does not catch the echo
case, because an echoed readback transcribes as the readback's own content
(*"Confirm booking for four thousand one hundred eighty dollars"*), which passes classification and
reads as affirmative. P2's case involves a human; this one does not. CON-02 does not reach it either
— CON-02 is about *response* identity, and this arrives through an acoustic channel the design does
not model.

**Evidence, not speculation.** The second prior implementation ships `looksLikeCurrentSpeechEcho`,
which exists for exactly this reason: it rejects a candidate transcript when it matches the
in-flight TTS by ordered-token overlap (≥3 tokens matched and ≥72% of candidate tokens), with higher
thresholds on mobile (4 final, 5 interim) because the acoustic path is worse.

**Why this is Phase 1 and not Phase 8.** Per D-09, `TransportCapabilities` is an interface consumers
**implement**. Widening it after publish is a real breaking change — unlike almost everything else in
this phase, the "cannot wait" premise genuinely applies here.

**Shape (provisional):** replace or supplement the boolean with a provenance value distinguishing at
minimum an identity the agent cannot forge (an explicit human act — button, click, keypress) from
one it can influence (recognizer-derived, or any channel the agent's own output feeds back into).
The planner should resist over-enumerating modalities; grades are modality-free and this must stay
so. The axis is *forgeable by the agent's own output* versus *not*, not *voice* versus *text*.

**Split of work:** Phase 1 makes provenance representable and type-tests it. Phase 8 implements the
runtime gate that refuses `bindTo: "userTurn"` on a forgeable transport. Traceability maps TRN-05 to
Phase 1 because that is where the breaking change is; Phase 8's notes record the gate.

### D-11 — Note only: CON-10 was minted from this discussion

Not Phase 1 work, recorded here so the planner does not treat it as missing scope. The second prior
implementation's `buildControlOutcome(modelText, results)` **discards the model's narration entirely
when any result failed** and speaks the app's own failure messages instead — the agent is
structurally not permitted to narrate a failure. That is a working, cheap mitigation of the ASI09
reauthoring problem, and Concierge had nothing between `delivered` and `attested` doing it. It is now
CON-10, mapped to Phase 8, with its mechanism at the Phase 7 session seam. Phase 1 ships no type for
it.

### D-12 — Open questions raised by research, resolved 2026-07-28

Research surfaced five decisions it could not make alone. Resolved here so the planner does not
re-open them:

1. **Declare `ReadbackAttestation` in Phase 1, or defer to Phase 8?** → **Defer to Phase 8.** D-09's
   breaking-change argument does not apply before publish, and Phase 8 is where the kernel actually
   requires it. Phase 1 ships the receipt types only.
2. **`any`- or `never`-erasure for `AnyActionDefinition`?** → **`any`**, with a doc comment. See D-07.
...
5. **`packages/concierge/package.json` needs one script line.** → **Land it early in the phase.**
   ...
   not worth restructuring the parallelization, but the Phase 2 planner should expect it.

**Two more items research flagged that belong in the plan:**

- **`README.md:72` renders `reason?: string`** — the only place in the docs referencing any type this
  phase changes. It needs a one-line correction, or the published README contradicts the shipped
  contract on day one.
- **Phase 1 must be planned as a single serial wave.** `config.json` sets `parallelization: true`,
  but every step edits the same 540-line `types.ts` and parallel waves would conflict on nearly
  every hunk. This is a per-phase override of a global setting — state it explicitly in the plan so
  the executor does not re-parallelize.
...
- **`voyza-voice-browser-control-spec.md`** — the original provenance recorded in PROJECT.md. 2685
  lines, 28 control actions across 6 stages, ~3,947 LOC non-test.
- **`portfolio` repository, branch `audit-fsb-ai-control-loop`** (2026-07-16, ~12.4k insertions,
  Next.js) — a *second, independent* shipped implementation, located during this discussion. Not in
  this repo; read via `git show audit-fsb-ai-control-loop:<path>` from a clone of `portfolio`.
  Highest-value files for a Phase 1 planner, in order:
  - `src/lib/control-result.ts` — `ControlResult {ok, message}`, the `CONTROL_FAILURE_MESSAGES`
    map that produced `invalid_result`, `sanitizeControlMessage` +
    `CONTROL_RESULT_MESSAGE_MAX_CHARS = 180` that produced SEC-06, `sanitizeControlDisplayArgs`
    (a shipped `RedactionPolicy` with a `drop` default), and `buildControlOutcome`, which produced
    CON-10.
  - `src/lib/voice-turn-policy.ts` — `shouldPersistVoiceTurn` (only user turns count, i.e.
    `bindTo: "userTurn"` shipped) and `looksLikeCurrentSpeechEcho`, which produced TRN-05 / D-10.
  - `src/lib/voice-commands.ts` — the interrupt-vs-end-session split, confirming `AbandonReason`'s
    `declined` / `cancelled` distinction is load-bearing rather than theoretical.
  - `src/lib/site-control-results.ts` — result construction at the boundary; every plan returns a
    `ControlResult` alongside its state change.

  **Caveat for the planner:** this system has no consent gate, no grades, no readback hash, no
  snapshot equality, and no dedup by reference identity. Treat it as corroboration of *shapes* and
  as a source of *failure modes it hit*, never as a design to match.
...
- ✅ **`readsUntrusted` build-time gate — filed as SEC-05, Phase 3** (done 2026-07-27; ROADMAP and
  REQUIREMENTS both updated). Open sub-questions for that requirement, to settle during Phase 3
  discussion: error vs warn, and whether `readOnly: true` combined with `readsUntrusted: true` also
  gates — ingress into model context is itself the injection vector, so "read-only" is not
  automatically safe.
- ✅ **`invalid_result` runtime half — filed as DSP-09, Phase 6.** Phase 1 ships the code; Phase 6
  ships the `normalizeResult` boundary that produces it.
- ✅ **Message sanitizer — filed as SEC-06, Phase 6.** Phase 1 exports `MESSAGE_MAX_CHARS`; Phase 6
  strips control characters, collapses whitespace, and truncates at the dispatcher boundary.
- ✅ **Agent may not narrate a failure — filed as CON-10, Phase 8** (mechanism at the Phase 7 session
  seam). See D-11.
- ✅ **TRN-05 runtime gate → Phase 8.** Phase 1 makes provenance representable; Phase 8 refuses
  `bindTo: "userTurn"` on a transport whose turn identity the agent can mint. See D-10.
