...
The complete implementation roadmap is preserved below. The exact source candidate `161dfb81c1141d498bee6a6130c86984c023f522` passed hosted certification in GitHub Actions run `31645153579`; milestone archival is a later planning-only successor and is not covered by that receipt.
...
- [x] **Phase 6: Dispatcher** - A retried, malformed, aborted, or crashing call produces exactly one honest result and never fires an effect twice (completed 2026-08-07)
...
- [x] **Phase 10: Close v0.1 release certification and evidence gaps** - Close all local audit gaps and certify one exact pre-publication candidate through a hosted Ubuntu receipt (completed 2026-08-12)
...
| 10. Close v0.1 release certification and evidence gaps | 7/7 | Complete | 2026-08-12 |
...
**Certification status:** Exact candidate `161dfb81c1141d498bee6a6130c86984c023f522` certified successfully by run `31645153579` (attempt 1); receipt SHA-256 `20bff8d8b40e99d3e414f91dc6d923ee8f5fc82e8bde21e5bd23554028afdfea`.
...
The terminal gate completed successfully for `161dfb81c1141d498bee6a6130c86984c023f522`. No repository write occurred between certification and the later, explicitly authorized milestone archival.

## Milestone Summary

**Key decisions:**

- Keep core vendor- and DOM-neutral while framework adapters own lifecycle only.
- Return the exact cached Promise for retry identity and compose session/catalog/stop cancellation under one serialized authority model.
- Arm one-shot consent only after completed delivery and bind it to canonical retained payload bytes plus a fresh human-origin turn.
- Keep core as a peer dependency of both adapters so one physical instance owns bridges, deduplication, and consent.
- Treat the hosted run-scoped receipt as the terminal certification fact; later repository writes require a new candidate.

**Issues resolved:** all 62 requirements, 12 integration edges, 10 end-to-end flows, Phase 9 Nyquist coverage, and the exact-SHA hosted gate.

**Issues deferred:** five acknowledged open artifacts are recorded in `STATE.md`; npm publication and registry provenance remain operational release work.

**Technical debt:** correct the over-broad shipped `sideEffects: false` explanation, complete the historical human-needed reviews, and retain the documented Node-floor helper and diagnostic-legibility follow-ups.
