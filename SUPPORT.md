...
Concierge 0.2 is a supported public preview. “Supported” means its documented
...
The 0.2 support contract includes:

- documented exports from the four public package export maps;
- runtime contract v2 and the documented signed-envelope v1 wire fields;
...
For `0.2.x` patches, maintainers will not intentionally break those surfaces.
...
Only the latest `0.2.x` patch receives fixes. The 0.2 line is supported until
the later of:

- six months after `0.2.0` is published; or
- 90 days after `0.3.0` is published.
