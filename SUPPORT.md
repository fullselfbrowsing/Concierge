...
Concierge 0.4 is a supported public preview. “Supported” means its documented
...
The 0.4 support contract includes:
...
For `0.4.x` patches, maintainers will not intentionally break those surfaces.
...
Only the latest `0.4.x` patch receives general fixes. The 0.4 line is supported until
the later of:

- six months after `0.4.0` is published; or
- 90 days after `0.5.0` is published.

The 0.3 line remains eligible for critical security fixes until 90 days after
0.4.0 is published. Applications may remain on 0.3 during that window, but must
not combine its contract-v3 core with 0.4 adapters.
