...
/** `Equals`, not `Assignable`, per `actions.test-d.ts:469-476`: an `Assignable` spelling "stays true when the field is widened to `unknown`, to a bare function type, or to a union that swallows the declared type — which is exactly the silent-widening regression worth guarding". CONTEXT settles the two-string form, so this shape is stable and a change to it should be a decision rather than a drive-by. The `ActionResult` return is pinned in the same breath: a helper that started returning a bare object literal would lose the closed `ReasonCode`, and `no_bridge` is the only reason code this phase ships a producer for. */
type _offPageResultSignature = Expect<Equals<typeof offPageResult, (what: string, where: string) => ActionResult>>;
